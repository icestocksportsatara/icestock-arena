...
| Layer | Stack |
|---|---|
| Backend API | Node.js, Express, PostgreSQL, Socket.io (live scoring), JWT auth, OTP email verification |
...
| Security | helmet CSP + HSTS, rate limiting, bcrypt, OTP two-step login, RBAC, audit log, input validation, XSS sanitization |
...
reachable right now, whether SMTP looks configured, and your configured
`CLIENT_URL`. No login needed, no secrets exposed. Start every
troubleshooting session here.
...
2. **Render's free tier is asleep.** After ~15 minutes of no traffic, the
   first request can take 30–60 seconds while it wakes up, and the OTP
   login flow makes two round trips, so a cold start can look like a
   failure on the first attempt. Fix: use a free uptime pinger like
   ...
   service warm.
3. **SMTP isn't configured, or the credentials are wrong.** Login will get
   past step 1 but fail with a clear "email delivery is not configured
   correctly" message (not a generic error) — see the OTP email setup
   below.
