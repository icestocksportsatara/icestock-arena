...
-- ---------------------------------------------------------------------------
-- OTP CODES — two-step login verification (password + one-time code)
-- ---------------------------------------------------------------------------
CREATE TABLE otp_codes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    code_hash TEXT NOT NULL,
    purpose VARCHAR(30) NOT NULL DEFAULT 'LOGIN', -- LOGIN | PASSWORD_RESET
...
-- ---------------------------------------------------------------------------
-- TOURNAMENT REGISTRAR ASSIGNMENTS
-- ---------------------------------------------------------------------------
-- Admin (SUPER_ADMIN) decides which Country/State/District Head accounts are
-- allowed to register teams/players INTO a given tournament. A head with no
-- assignment row for a tournament cannot enter participants into it, even if
-- the tournament is inside their normal geographic scope. This is what makes
-- registration "per assigned tournament" rather than blanket access.
CREATE TABLE tournament_registrars (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tournament_id UUID NOT NULL REFERENCES tournaments(id) ON DELETE CASCADE,
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_registrars_tournament ON tournament_registrars(tournament_id);
CREATE INDEX idx_registrars_user ON tournament_registrars(user_id);

-- A team or player "entering" a specific tournament (separate from the
-- team/player existing in the system generally — the same team can enter
-- multiple tournaments over a season).
