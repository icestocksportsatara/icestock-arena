...
/** Link an existing player record to a login account (creates PLAYER user first via /api/users). */
router.patch('/:id/link-user', requireRole(...REGISTRARS), [body('userId').isUUID()], validate, async (req, res, next) => {
  try {
